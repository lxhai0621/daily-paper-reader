...
tags: ["query:ma-kf"]
score: 8.0
evidence: 通过提示卡标准化提示词文档化与评估
tldr: 随着大语言模型的普及，提示工程变得日益复杂，但目前缺乏标准化的文档和评估实践。本文提出了“提示卡（Prompt Cards）”概念，借鉴模型卡片的思路，为提示工程提供结构化的总结。通过记录提示意图、上下文策略、评估方法和伦理考量，提示卡增强了提示过程的透明度与可复现性，为评估生成文本质量提供了除基准测试外的有效替代方案。
selection_source: carryover_cache
motivation: 提示工程领域目前缺乏标准化的文档记录和评估实践，导致复杂的提示设计过程难以被理解、复现和系统性评估。
...
conclusion: 提示卡通过使隐性的设计决策显性化，为提升提示工程的复现性、透明度和方法论提供了重要的标准化工具。
---

## 摘要
大语言模型（LLMs）的飞速发展和卓越能力催生了提示工程（prompt engineering）领域，即通过精心设计输入来引导 LLMs 生成高质量且与任务相关的输出。该领域面临的一个关键挑战是缺乏标准化的提示文档记录和评估实践。提示词可能冗长、复杂，且在主观任务上难以评估。为了应对这一挑战，我们建议使用提示卡（prompt cards），这是一种受模型卡（model cards）概念启发而设计的提示工程实践结构化摘要。通过提示卡，可以系统地记录和评估提示工程过程中的具体目标、考量因素和所采取的步骤。我们展示了提示卡方法，并将其应用于一项名为“文字化”（wordalisation）的具体任务中，该任务旨在将结构化数值数据转化为文本。我们认为，结构良好的提示卡能够实现更好的可复现性和透明度，改进提示方法论，并为评判生成文本质量提供一种替代基准测试（benchmarking）的有效手段。通过系统地捕捉底层模型细节、提示意图、上下文策略、评估实践和伦理考量，提示卡使塑造系统行为的、通常是隐性的设计决策变得显性化。记录这些选择至关重要，因为提示过程正日益涉及具有多个动态环节的复杂流水线。
