...
tldr: 针对多模态大语言模型在推理中存在的计算资源与任务难度不匹配问题，本文提出了一种覆盖率感知的多模态解码方法（CAMD）。该方法基于采样覆盖率、实例难度与残余风险的理论框架，通过动态分配计算资源来处理不同难度的样本。CAMD集成了证据加权评分、后验覆盖估计和序列贝叶斯更新，在有限的Token预算下实现了效率与可靠性的平衡，显著提升了模型在各类基准测试中的表现。
selection_source: fresh_fetch
motivation: 现有的解码方法在简单任务上浪费计算资源，而在复杂任务上投入不足，导致效率与效果难以兼得。
method: 提出CAMD机制，利用证据加权评分和贝叶斯更新，根据估计的不确定性动态调整推理时的计算量。
result: 在多个基准数据集上的实验证明，CAMD在有限的计算预算下显著提高了多模态推理的准确性和效率。
conclusion: 通过自适应分配计算资源，CAMD有效解决了多模态推理中的长尾难度分布问题，为高效可靠的MLLM推理提供了新方案。
---

## 摘要
多模态大语言模型（MLLMs）的最新进展在视觉-语言任务中展现了令人印象深刻的推理能力，但仍面临计算量与难度不匹配的挑战。通过实证分析，我们发现现有的解码方法可能会在简单案例上浪费计算资源，而在困难案例上投入不足，从而影响模型的有效性和效率。为解决这一问题，我们首先开发了一个理论框架，将采样覆盖范围、实例难度和残余风险联系起来。我们的分析表明，多模态推理呈现出重尾难度分布；一小部分困难或模糊的样本主导了残余失败概率。基于这一见解，我们提出了覆盖感知多模态解码（CAMD），这是一种根据估计的不确定性动态分配计算资源的自适应推理机制。CAMD 集成了证据加权评分、后验覆盖估计和序列贝叶斯更新，以在有限的 token 预算下平衡效率和可靠性。在各种基准数据集和基准模型上的实验证明了我们方法的有效性和优势。
