...
## 摘要
大语言模型（LLM）向具备工具调用能力的自主智能体的快速演进，从根本上改变了网络安全格局。诸如 OpenClaw 之类的框架赋予了 AI 系统操作系统级的权限以及执行复杂工作流的自主权。这种级别的访问权限带来了前所未有的安全挑战。因此，传统的内容过滤防御手段已变得过时。本报告对 OpenClaw 生态系统进行了全面的安全分析。我们系统地调查了其当前的威胁态势，重点分析了关键漏洞，如提示词注入驱动的远程代码执行（RCE）、序列化工具攻击链、上下文失忆以及供应链污染。为了系统地梳理这些威胁，我们为自主智能体提出了一种新型的三层风险分类法，从 AI 认知、软件执行和信息系统三个维度对漏洞进行分类。针对这些系统性的架构缺陷，我们引入了全生命周期智能体安全架构（FASA）。这一理论防御蓝图倡导零信任智能体执行、动态意图验证以及跨层推理-动作关联。基于该框架，我们展示了正在进行的工程项目 Project ClawGuard。该项目旨在实现 FASA 范式，并将自主智能体从高风险的实验性工具转变为可信系统。我们的代码和数据集可在 https://github.com/NY1024/ClawGuard 获取。
