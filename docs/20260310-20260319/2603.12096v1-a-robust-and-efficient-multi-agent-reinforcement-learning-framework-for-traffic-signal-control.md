...
tldr: 针对交通信号控制中强化学习泛化性差和动作空间不匹配的问题，本文提出一种鲁棒高效的多智能体强化学习（MARL）框架。该框架通过转向率随机化增强鲁棒性，采用指数级相位时长调整优化稳定性，并结合邻域观测的MAPPO算法实现协同控制。在Vissim仿真中，该方法显著降低了平均等待时间，展现出优异的泛化能力和控制稳定性。
selection_source: fresh_fetch
motivation: 现有的交通信号强化学习方法在应对动态交通流时泛化能力不足，且动作空间设计往往不符合驾驶预期。
method: 提出包含转向率随机化训练、指数级相位时长调整动作空间以及基于邻域观测的MAPPO算法的集成框架。
result: "实验结果显示，该框架在多种交通场景下将平均等待时间降低了10%以上，且具有更强的鲁棒性。"
conclusion: 该研究为自适应交通信号控制提供了一种兼具扩展性、稳定性和泛化能力的实用化解决方案。
---

## 摘要
交通信号控制（TSC）中的强化学习（RL）在实际部署中面临重大障碍，原因是对动态交通流变化的泛化能力有限。现有方法通常过拟合静态模式，且使用的动作空间与驾驶员预期不符。本文提出了一种在 Vissim 交通模拟器中验证的鲁棒多智能体强化学习（MARL）框架。该框架集成了三种机制：(1) 转向比随机化，这是一种通过让智能体接触动态转向概率来增强对未知场景鲁棒性的训练策略；(2) 面向稳定性的指数相位时长调整动作空间，通过周期性的指数相位调整来平衡响应速度和精度；(3) 基于邻域的观测方案，利用带有集中式训练分布式执行（CTDE）的 MAPPO 算法。通过利用集中式更新，该方法在保持可扩展本地通信的同时，接近了全局观测的效果。实验结果表明，我们的框架优于标准的强化学习基准，将平均等待时间减少了 10% 以上。所提出的模型在未知交通场景中表现出卓越的泛化能力，并保持了较高的控制稳定性，为自适应信号控制提供了一种实用的解决方案。
