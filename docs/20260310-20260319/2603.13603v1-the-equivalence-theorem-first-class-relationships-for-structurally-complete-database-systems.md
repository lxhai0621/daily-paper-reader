...
tags: ["query:ma-kf"]
score: 6.0
evidence: 结构完整数据库系统的知识表示框架
tldr: 本研究提出了“等价定理”，证明结构完备的知识表示必须具备n元关系属性、时间有效性、不确定性量化和因果关系这四项互补能力，且这等同于将关系视为一等公民。研究构建了ATCH框架，确立了从SQL到ATCH的表达力层级，并解决了框架问题等经典AI难题。通过PostgreSQL扩展验证了其在处理复杂查询时的实用性与计算可行性。
selection_source: fresh_fetch
motivation: 旨在解决现有数据库系统在处理复杂知识表示时缺乏结构完备性的问题。
method: 提出等价定理并构建了集属性、时间、因果于一体的超图（ATCH）框架。
result: 确立了严格的表达力层级，证明了ATCH在处理特定查询时优于SQL和图数据库，并给出了计算复杂度界限。
conclusion: 证明了将关系作为一等对象处理是实现结构完备数据库系统的必要且充分条件。
---

## 摘要
我们证明了等价定理：结构完备的知识表示需要恰好四种相互蕴含的能力——带属性的 n 元关系、时间有效性、不确定性量化以及关系之间的因果关系——这些能力共同等同于将关系视为一等对象（first-class objects）。任何实现其中一种能力的系统必然需要全部四种；任何缺失其中一种的系统都无法实现结构完备性。这一结果是构造性的：我们展示了一个同时满足所有四个条件的属性时间因果超图（ATCH）框架。该定理产生了一个严格的表达能力层级——SQL < LPG < TypeDB < ATCH——并给出了在每个较低层级上结构上无法表达的见证查询。我们建立了计算复杂度界限，表明通用查询具有 NP 完全性，但对于实际查询类（无环模式、有限深度因果链、窗口化时间查询）具有多项式时间可处理性。作为直接推论，我们推导出了经典人工智能问题的解决方案：框架问题（由时间有效性产生的默认持久性）、冲突解决（作为带有隐变量发现的未解决元数据的矛盾）以及常识推理（带有因果抑制因子的默认值）。一个用 C 语言编写的 PostgreSQL 原型扩展在既定的复杂度范围内验证了实际可行性。
