...
tags: ["query:ma-kf"]
score: 8.0
evidence: 智能体系统的安全威胁与防御
tldr: 本文探讨了从安全智能体AI向安全智能体Web演进中的安全挑战。随着大模型具备规划、记忆和行动能力，其安全风险从单纯的文本生成扩展到工具调用、持久记忆和环境交互。文章提出了涵盖提示词滥用、环境注入、工具链滥用等维度的威胁分类法，综述了防御策略，并深入分析了Web规模下委托链和跨域交互带来的风险放大效应，为构建可信智能体生态指明了研究方向。
selection_source: fresh_fetch
motivation: 大模型智能体在开放环境中的广泛应用使得安全风险从文本生成转向了更具破坏性的工具调用和环境交互。
method: 通过系统性综述，构建了涵盖组件威胁、防御策略及Web规模演进的分类框架，并分析了复杂生态下的风险传播机制。
result: 总结了从提示词硬化到协议级安全机制的多种防御手段，并识别了身份验证、溯源及自适应对抗评估等关键挑战。
conclusion: 构建安全的智能体Web需要跨学科协作，在协议、系统和生态层面建立互操作的身份授权与持续的安全监控机制。
---
