...
tags: ["query:agent"]
score: 7.0
evidence: 个性化LLM智能体的持久记忆管理与审计
tldr: 该论文针对个性化语言智能体使用持久记忆适应用户时，难以区分真实偏好漂移与临时上下文变化、歧义或对抗性记忆投毒的问题，将其形式化为潜在用户状态上的连续时间部分可观测决策过程。CAPTURE方法结合神经常微分方程信念追踪、多时间尺度记忆账本、不确定性触发的澄清与对引用记忆的反事实审计。在96名用户的480个留出回合上评估，验证其有效性。贡献在于为智能体长期记忆的安全与可信管理提供机制。
source: arxiv
...
motivation: 个性化LLM智能体的持久记忆管理与审计。
