...
## 摘要
AI 记忆系统越来越多地将知识组织成图结构——如知识图谱、实体关系、社区层级——但仍缺乏一种原则性的连续分辨率控制机制：抽象层级之间的定性边界在哪里，智能体又该如何导航这些边界？我们引入了语义细节层次（SLoD），这是一个通过在庞加莱球 $\mathbb{B}^d$ 上定义热核扩散的连续缩放算子来回答上述两个问题的框架。在粗尺度（$σ\to \infty$）下，扩散将嵌入聚合为高层摘要；在细尺度（$σ\to 0$）下，局部语义细节得以保留。我们证明了在 Sarkar 嵌入下，树状层级结构具有有界近似误差 $O(σ)$ 和 $(1+\varepsilon)$ 失真度的层级一致性。至关重要的是，我们展示了图拉普拉斯算子中的谱隙会诱导出涌现的尺度边界——即表示发生定性转变的尺度——这些边界可以在没有手动分辨率参数的情况下自动检测。在合成层级（HSBM）上，我们的边界扫描器恢复预设层级的 ARI 高达 1.00，且在接近信息论 Kesten-Stigum 阈值时，检测性能平稳下降。在完整的 WordNet 名词层级（8.2万个同义词集）上，检测到的边界与真实的分类深度一致（$τ= 0.79$），证明了该方法能够在无监督的情况下发现真实世界知识图谱中具有意义的抽象层级。
