...
tags: ["query:ma-kf"]
score: 8.0
evidence: 针对工具增强型 LLM 智能体的检索层攻击
tldr: "本研究提出ToolFlood，一种针对大模型智能体工具检索阶段的新型攻击方法。不同于以往针对工具选择的攻击，ToolFlood通过在嵌入空间中注入少量精心设计的恶意工具，利用语义覆盖策略占据检索结果的前k个位置，从而将合法工具挤出上下文。该方法结合了LLM生成多样化工具元数据和贪心选择算法，在仅注入1%恶意工具的情况下，在ToolBench等基准测试中实现了高达95%的攻击成功率，揭示了工具增强型LLM系统在检索层面的严重安全漏洞。"
selection_source: carryover_cache
motivation: 现有研究多关注工具选择阶段的攻击，而忽略了工具检索层在面对语义覆盖攻击时的鲁棒性问题。
method: 通过LLM生成多样化的工具描述，并利用迭代贪心算法选择能最大化覆盖用户查询嵌入空间的恶意工具。
result: "在ToolBench实验中，仅需1%的注入率即可达到最高95%的攻击成功率，有效屏蔽了所有合法工具。"
conclusion: ToolFlood证明了基于嵌入的工具检索机制极易受到语义饱和攻击，凸显了加强工具增强型LLM系统检索层防御的必要性。
---

## 摘要
大语言模型（LLM）智能体越来越多地使用外部工具来处理复杂任务，并依赖基于嵌入的检索来选择一小部分 top-k 子集进行推理。随着这些系统的规模扩大，尽管之前的研究已经探讨了针对工具选择的攻击，但该检索阶段的鲁棒性尚未得到充分研究。本文介绍了 ToolFlood，这是一种针对工具增强型 LLM 智能体的检索层攻击。ToolFlood 并非在检索后改变所选工具，而是通过注入少量由攻击者控制的工具来使检索过程本身过载，这些工具的元数据是通过利用嵌入空间的几何特性精心布置的。这些工具在语义上覆盖了许多用户查询，占据了 top-k 结果的主导地位，并将所有良性工具挤出了智能体的上下文。ToolFlood 采用了一种两阶段的对抗性工具生成策略：首先对目标查询子集进行采样，并使用 LLM 迭代生成多样化的工具名称和描述；然后运行迭代贪婪选择，在余弦距离阈值下选择能最大化覆盖嵌入空间中剩余查询的工具，直到所有查询都被覆盖或达到预算上限。我们提供了检索饱和度的理论分析，并在标准基准测试上表明，ToolFlood 在较低的注入率下（在 ToolBench 中为 1%）可实现高达 95% 的攻击成功率。代码将在以下链接公开：https://github.com/as1-prog/ToolFlood
