...
motivation: 现有的LLM智能体安全测试多关注单工具或单跳，忽略了多工具组合中因数据持久化和重用产生的跨工具长路径漏洞。
method: 提出ChainFuzzer框架，通过提取跨工具数据流链、利用TPS合成执行提示词以及进行护栏感知的变异模糊测试来发现漏洞。
result: "在20个开源智能体应用中确认了365个唯一且可复现的漏洞，其中302个涉及多工具执行，且TPS将链的可达性从27.05%提升至95.45%。"
conclusion: ChainFuzzer为测试和加固现实世界中的多工具智能体系统提供了一个高效且实用的基础框架。
---
