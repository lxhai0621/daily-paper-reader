...
motivation: 现有的生成式检索模型在处理涉及数值和复杂语义推理的查询时能力有限，导致检索准确率较低。
method: 提出了 ReasonGR 框架，采用结构化提示策略引导逐步推理，并结合推理聚焦的适配模块来优化相关参数学习。
result: 在 FinQA 财务查询数据集上的实验表明，ReasonGR 显著提升了检索的准确性和逻辑一致性。
conclusion: ReasonGR 证明了通过增强多步语义推理能力，可以有效提升生成式检索模型在复杂应用场景下的性能。
---
