...
tags: ["query:ma-kf"]
score: 8.0
evidence: 联邦学习中用于检索、规划和协调的专业智能体
tldr: 本研究针对6G网络中用户定制化设备端学习的需求，提出了一种基于智能体AI（Agentic AI）的控制平面智能层，用于管理联邦学习（FL）。该系统将FL视为学习与网络管理的结合任务，通过检索、规划、编码和评估等专业智能体，利用监控工具和优化方法处理客户端选择、资源分配及自适应训练。通过闭环评估和记忆机制，系统能根据网络状况和设备能力持续优化决策，显著提升了6G环境下联邦学习的性能。
selection_source: fresh_fetch
motivation: 6G网络下分布式数据训练面临严苛的延迟、带宽和可靠性挑战，需要更智能的网络管理方案来应对复杂的通信环境。
method: 提出一种由多专业智能体组成的Agentic AI系统，作为控制层将高层任务目标转化为感知网络状况的执行动作。
result: 案例研究表明，该系统通过调用优化工具和闭环反馈机制，在多变的网络环境下实现了高效的联邦学习性能。
conclusion: Agentic AI为6G联邦学习提供了一个智能化的控制平面，有效实现了学习任务与网络资源的协同优化。
---

## 摘要
向用户定制化端侧学习的转变对无线系统提出了新要求：模型必须在多样化、分布式数据上进行训练，同时满足严格的延迟、带宽和可靠性约束。为此，我们提出将代理式 AI 作为管理 6G 网络联邦学习（FL）的控制层，该层能将高层任务目标转化为感知网络状况的行动。我们的系统并非简单地将 FL 视为一个学习挑战，而是将其视为学习与网络管理的结合任务。一组专注于检索、规划、编码和评估的专业智能体利用监控工具和优化方法，来处理客户端选择、激励机制构建、调度、资源分配、自适应本地训练以及代码生成。闭环评估和记忆机制的使用使系统能够持续优化其决策，并兼顾不断变化的信噪比、带宽条件和设备能力。最后，我们的案例研究证明了代理式 AI 系统通过使用工具实现高性能的有效性。
