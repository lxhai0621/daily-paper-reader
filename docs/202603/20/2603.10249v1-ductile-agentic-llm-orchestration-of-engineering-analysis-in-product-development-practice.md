...
method: 提出DUCTILE框架，通过LLM智能体解释设计规范并编排工具执行，实现自适应编排与确定性执行的分离。
result: 在航空航天工业案例中，该智能体能成功处理格式、单位和命名等输入偏差，生成符合专家标准的分析结果。
conclusion: 智能体编排能有效提升工程自动化的灵活性，但也带来了从执行者转变为监督者的工作性质转变及潜在的监督疲劳挑战。
---
