...
motivation: 现有的幻觉抑制方法多关注文本诱导的幻觉，而忽视了视觉模态引起的失真问题。
method: 通过扩散模型生成与原描述矛盾的反事实图像，提取幻觉特征子空间，并在推理时对隐藏状态进行投影修正。
result: 在多个基准测试中，CIPHER显著降低了幻觉发生率，并有效保留了模型的原有任务性能。
conclusion: 利用反事实视觉扰动来识别并抑制特征层面的幻觉，是提升LVLM忠实度的一种高效且无需训练的途径。
---

## 摘要
尽管大语言视觉模型（LVLM）在多模态任务上取得了强劲性能，但它们经常产生幻觉——即与视觉输入不一致的不忠实输出。为解决这一问题，我们提出了 CIPHER（用于幻觉提取与消除的反事实图像扰动），这是一种无需训练的方法，通过轻量级的特征级修正来抑制视觉诱导的幻觉。与以往主要关注文本诱导幻觉的无需训练方法不同，CIPHER 明确针对源自视觉模态的幻觉。CIPHER 分两个阶段运行。在离线阶段，我们构建了 OHC-25K（物体幻觉反事实数据集，包含 25,000 个样本），这是一个由扩散编辑图像组成的反事实数据集，这些图像刻意与原始真实标注（ground-truth captions）相矛盾。我们将这些编辑后的图像与未改变的真实标注配对，并通过 LVLM 处理以提取与幻觉相关的表示。将这些表示与来自真实（图像，标注）对的表示进行对比，揭示了跨越低秩子空间的结构化、系统性偏移，该子空间刻画了视觉诱导的幻觉。在推理阶段，CIPHER 通过将中间隐藏状态投影到远离该子空间的方向来抑制幻觉。在多个基准测试上的实验表明，CIPHER 在保持任务性能的同时显著降低了幻觉率，证明了反事实视觉扰动在提高 LVLM 忠实度方面的有效性。代码和补充材料可在 https://hamidreza-dastmalchi.github.io/cipher-cvpr2026/ 获取。
