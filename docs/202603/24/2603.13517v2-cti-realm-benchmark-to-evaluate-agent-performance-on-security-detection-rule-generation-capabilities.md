...
tags: ["query:agent"]
score: 8.0
evidence: 评估AI智能体决策与规则生成能力的基准测试
...
## 摘要
CTI-REALM（网络威胁真实世界评估与大语言模型基准测试）是一个旨在评估 AI 智能体解读网络威胁情报（CTI）并开发检测规则能力的基准。该基准提供了一个模拟安全分析师工作流的真实环境，使智能体能够审查 CTI 报告、执行查询、理解模式结构并构建检测规则。评估涉及在 Linux 系统、云平台和 Azure Kubernetes 服务（AKS）上进行的各种复杂程度的模拟攻击，并配有地面真值数据以进行准确评估。智能体的表现通过最终检测结果和捕捉决策有效性的基于轨迹的奖励来衡量。这项工作展示了 AI 智能体在支持检测工程中劳动密集型环节方面的潜力。我们对 16 个前沿模型的全面评估显示，Claude Opus 4.6 (High) 获得了最高的总奖励（0.637），其次是 Claude Opus 4.5 (0.624) 和 GPT-5 系列。消融实验证实，针对 CTI 的特定工具显著提升了智能体的表现，而跨多次运行的方差分析则证明了结果的稳定性。最后，一项记忆增强研究表明，种子上下文可以弥合小型模型与大型模型之间 33% 的性能差距。
