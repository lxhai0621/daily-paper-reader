...
tags: ["query:ma-kf"]
score: 8.0
evidence: 针对真实工作流中智能体AI的可信度评估
tldr: 针对智能体AI在复杂现实任务中面临的信任风险，本文指出当前评估方法过于碎片化，缺乏代表性。为此提出了全息智能体评估框架（HAAF），通过多维场景流形、交互式沙盒模拟及分布感知采样，系统性地评估智能体在任务、工具、社交及风险维度的表现，并通过迭代优化工厂提升其安全性，实现了从孤立基准测试向真实世界代表性评估的转变。
selection_source: carryover_cache
motivation: 当前智能体评估侧重于孤立能力的测试，缺乏对复杂现实场景中系统性风险和代表性分布的考量。
method: 提出HAAF框架，整合静态分析、沙盒模拟、伦理对齐及分布感知采样引擎，并通过红蓝对抗的优化工厂进行迭代加固。
result: 该框架能够有效覆盖高风险长尾场景，通过系统化的场景流形刻画，提供了比传统基准测试更全面的信任度评估。
conclusion: HAAF为智能体AI提供了一种从“基准孤岛”转向具有现实代表性的信任评估范式，有助于推动安全可靠的智能体部署。
---

## 摘要
随着代理式 AI 系统从静态问答转向开放式、工具增强且多步骤的现实世界工作流，其权限的提升带来了更大的系统滥用和操作失败风险。然而，当前的评估实践仍然是碎片化的，仅在狭隘定义的设置中测量孤立的能力，如编程、幻觉、抗越狱性或工具使用。我们认为，核心局限性不仅在于评估维度的覆盖不足，还在于缺乏原则性的代表性概念：智能体的可信度应该在具有代表性的社会技术场景分布上进行评估，而不是在一组相互孤立的基准实例上进行评估。为此，我们提出了全息智能体评估框架（HAAF），这是一种系统性的评估范式，它在涵盖任务类型、工具接口、交互动态、社会背景和风险水平的场景流形上刻画智能体的可信度。该框架集成了四个互补组件：(i) 静态认知与策略分析，(ii) 交互式沙盒模拟，(iii) 社会伦理对齐评估，以及 (iv) 分布感知的代表性采样引擎，该引擎共同优化了覆盖范围和风险敏感性——特别是针对传统基准测试系统性忽视的罕见但后果严重的尾部风险。这些组件通过一个迭代的可信优化工厂相连接。通过红队探测和蓝队加固的循环，该范式逐步缩小漏洞以满足部署标准，将智能体评估从基准孤岛转向具有代表性的现实世界可信度。示例实例的代码和数据可在 https://github.com/TonyQJH/haaf-pilot 获取。
