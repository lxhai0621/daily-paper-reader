...
tags: ["query:ma-kf"]
score: 7.0
evidence: 自动化网页信息提取基准
tldr: 针对传统网页信息提取（WIE）基准测试依赖静态快照、难以应对动态网页演变的问题，本文提出了LiveWeb-IE，一个直接针对实时网站进行评估的新型基准。该基准包含多种数据类别和四个复杂度等级的自然语言查询。同时，研究者提出了一种模拟人类认知过程的多阶段智能体框架VGS，通过视觉定位提取信息。实验证明VGS在多种模型上均表现出优异的有效性和鲁棒性，为开发实用的WIE系统奠定了基础。
selection_source: fresh_fetch
motivation: 传统的静态HTML快照基准无法反映网页随时间动态变化的特性，导致模型在现实场景中泛化能力不足。
method: 提出了LiveWeb-IE实时基准测试集，并开发了模拟人类视觉认知过程的多阶段智能体框架Visual Grounding Scraper (VGS)。
result: 实验结果表明，VGS框架在不同骨干模型上均展现出显著的提取效果和强大的鲁棒性。
conclusion: 本研究通过引入实时评估基准和创新的视觉定位框架，为构建实用且稳健的网页信息提取系统提供了重要参考。
---

## 摘要
网页信息抽取（WIE）是从网页中自动提取数据的任务，为各种应用提供了极高的实用价值。传统上，WIE 系统的评估依赖于基于单一时间点捕获的 HTML 快照构建的基准测试。然而，这种离线评估范式未能考虑到网页随时间演变的特性；因此，在这些静态基准测试上的表现往往难以泛化到动态的现实场景中。为了弥补这一差距，我们推出了 LiveWeb-IE，这是一个旨在直接针对实时网站评估 WIE 系统的新基准。基于受信任且获得许可的网站，我们策划了需要提取各种数据类别（如文本、图像和超链接）信息的自然语言查询。我们进一步根据待提取属性的数量和基数，将这些查询设计为四个复杂度等级，从而实现对 WIE 系统的细粒度评估。此外，我们提出了视觉定位抓取器（Visual Grounding Scraper, VGS），这是一种新型的多阶段智能体框架，它通过视觉缩小网页内容范围来模拟人类认知过程，从而提取所需信息。在多种骨干模型上进行的广泛实验证明了 VGS 的有效性和鲁棒性。我们相信，这项研究为开发实用且鲁棒的 WIE 系统奠定了基础。
