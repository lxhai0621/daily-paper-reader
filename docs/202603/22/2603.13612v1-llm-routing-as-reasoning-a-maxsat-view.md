...
tags: ["query:ma-kf"]
score: 6.0
evidence: 基于自然语言反馈和约束的LLM路由选择
tldr: 针对大语言模型（LLM）路由中用户偏好难以表达和模型属性不透明的问题，本文提出一种基于约束的解释方法。研究者将语言驱动的路由建模为加权MaxSAT/MaxSMT问题，将自然语言反馈转化为模型属性上的软硬约束。通过在25个模型上的实验，证明了该方法能有效根据反馈生成接近可行的推荐集，将路由过程转化为结构化约束优化问题。
selection_source: fresh_fetch
motivation: 现有的LLM路由难以处理用户以自然语言表达的复杂偏好以及模型属性的部分可观测性。
method: 将LLM路由建模为加权MaxSAT/MaxSMT问题，利用自然语言反馈诱导模型属性的硬约束和软约束。
result: 在25个模型的基准测试中，该方法能根据语言反馈产生接近可行的推荐集合，并揭示了无反馈场景下的系统先验。
conclusion: LLM路由可以被理解为在语言条件偏好下的结构化约束优化过程。
---
