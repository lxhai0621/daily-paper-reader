...
## 摘要
前沿大语言模型的最新进展使得代码审查智能体能够在开放式、推理密集型的场景中运行。然而，由于缺乏标准化的基准测试和细粒度的评估协议，除了粗略的成功指标外，很难评估代码审查智能体的行为，特别是对于误报成本高昂的任务。为了填补这一空白，我们推出了 CR-Bench（一个基准测试数据集）和 CR-Evaluator（一个针对代码审查智能体的细粒度评估流水线）。利用这些工具，我们进行了一项初步研究，在两个前沿模型上评估了单次（single-shot）智能体和基于 Reflexion 的智能体。我们发现，当代码审查智能体被设计为识别所有隐藏问题时，可能会表现出较低的信噪比；如果仅通过解决率来衡量，这会掩盖真实的进展和开发者的生产力。我们的分析确定了问题解决与虚假发现之间隐藏的权衡，揭示了制约有效智能体设计的边界。总之，随着基于大语言模型的系统从受控的基准测试转向真实世界的软件工程工作流，CR-Bench 和 CR-Evaluator 为研究和开发代码审查智能体提供了及时的基础。
