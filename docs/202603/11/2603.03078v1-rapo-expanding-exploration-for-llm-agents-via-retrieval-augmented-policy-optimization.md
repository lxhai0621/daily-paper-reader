...
tldr: 针对大语言模型智能体在强化学习中探索受限的问题，本文提出检索增强策略优化（RAPO）框架。该框架通过引入外部检索机制，在训练中显式扩展探索空间。RAPO包含混合策略智能体回滚和检索感知策略优化两个阶段，允许智能体在推理过程中参考检索到的细粒度步骤轨迹，并通过重要性采样和检索奖励校准策略梯度。实验证明，RAPO在多个推理任务上显著提升了性能和训练效率。
selection_source: fresh_fetch
motivation: 现有的智能体强化学习方法主要依赖同策略探索，限制了智能体发现新推理视角的能力，且缺乏细粒度的步骤级探索动态。
method: 提出RAPO框架，通过混合策略回滚引入检索到的外部步骤轨迹，并利用检索感知优化机制校准策略梯度。
result: "在三个智能体推理任务的14个数据集上，RAPO实现了5.0%的平均性能提升，且训练速度提高了1.2倍。"
conclusion: RAPO通过将检索机制与强化学习深度结合，有效扩展了智能体的探索视野，为提升大模型智能体的复杂推理能力提供了高效的新方案。
---
