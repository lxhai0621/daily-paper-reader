...
method: 提出 stratum 架构，通过将流水线执行与规划推理分离，将批量流水线编译为优化执行图，并利用 Rust 编写的高性能运行时进行异构后端执行。
result: 初步实验结果显示，stratum 在处理大规模智能体流水线搜索时，相比现有方案可实现高达 16.6 倍的加速。
conclusion: stratum 成功解决了智能体驱动的 ML 工作负载与传统系统基础设施之间的不匹配问题，为自动化机器学习搜索提供了高效的系统级解决方案。
---
