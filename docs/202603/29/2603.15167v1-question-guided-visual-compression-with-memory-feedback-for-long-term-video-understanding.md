...
tags: ["query:agent"]
score: 8.0
evidence: 多模态模型中用于长期视频理解的记忆反馈
tldr: 针对长视频理解中独立压缩帧导致无法捕捉完整事件的问题，本文提出QViC-MF框架。该框架打破了从感知到记忆的单向模式，引入记忆反馈机制，通过问题引导的多模态选择性注意力（QMSA）提取与问题相关的当前及历史视觉信息。实验证明，该方法在MLVU、LVBench等多个长视频基准测试中显著超越了现有SOTA模型，有效增强了对复杂时序事件的理解。
selection_source: carryover_cache
motivation: 传统的视觉压缩方法通常独立处理每一帧，导致模型难以理解长视频中的完整事件和复杂的时序关系。
method: 提出QViC-MF框架，利用问题引导的多模态选择性注意力（QMSA）实现记忆反馈驱动的迭代视觉压缩。
result: 在MLVU、LVBench、VNBench Long和VideoMME Long等多个长视频基准测试中均取得了显著的性能提升。
conclusion: 通过引入记忆反馈和问题引导机制，可以有效提升大模型对长视频中复杂事件和时序逻辑的理解能力。
---
